    ...
    <nav aria-label="Table pagination">
        <ul class="pagination pagination-sm mb-0">
            ...
            @if($pagination['current_page'] > 1)
                <li class="page-item">
                    <a class="page-link" href="?page={{ $pagination['current_page'] - 1 }}&per_page={{ $pagination['per_page'] }}{{ isset($queryString) ? '&' . $queryString : '' }}" aria-label="Previous">
            ...
            @if($start > 1)
                <li class="page-item">
                    <a class="page-link" href="?page=1&per_page={{ $pagination['per_page'] }}{{ isset($queryString) ? '&' . $queryString : '' }}">1</a>
                </li>
                @if($start > 2)
                    <li class="page-item disabled">
            ...
            @for($i = $start; $i <= $end; $i++)
                ...
                @else
                    <li class="page-item">
                        <a class="page-link" href="?page={{ $i }}&per_page={{ $pagination['per_page'] }}{{ isset($queryString) ? '&' . $queryString : '' }}">{{ $i }}</a>
                    </li>
                @endif
            @endfor
            ...
            @if($end < $pagination['last_page'])
                ...
                @endif
                <li class="page-item">
                    <a class="page-link" href="?page={{ $pagination['last_page'] }}&per_page={{ $pagination['per_page'] }}{{ isset($queryString) ? '&' . $queryString : '' }}">{{ $pagination['last_page'] }}</a>
                </li>
            @endif
            ...
            @if($pagination['current_page'] < $pagination['last_page'])
                <li class="page-item">
                    <a class="page-link" href="?page={{ $pagination['current_page'] + 1 }}&per_page={{ $pagination['per_page'] }}{{ isset($queryString) ? '&' . $queryString : '' }}" aria-label="Next">
