...
<div class="{{ $class }} mb-3">
  ...
  @if ($type === 'text' || $type === 'email' || $type === 'date' || $type === 'number')
    <input type="{{ $type }}" class="form-control @error($name) is-invalid @enderror" id="{{ $name }}"
      name="{{ $name }}" value="{{ $value ?? old($name) }}" @if ($required) required @endif
  ...
  @elseif($type === 'textarea')
    <textarea class="form-control @error($name) is-invalid @enderror" id="{{ $name }}" name="{{ $name }}"
      rows="{{ $rows }}" @if ($placeholder) placeholder="{{ $placeholder }}" @endif>{{ $value ?? old($name) }}</textarea>
  @elseif($type === 'checkbox')
    <div class="form-check">
      <input class="form-check-input @error($name) is-invalid @enderror" type="checkbox" id="{{ $name }}"
        name="{{ $name }}" value="{{ $checkboxValue }}"
