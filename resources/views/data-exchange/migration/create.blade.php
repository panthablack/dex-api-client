        ...
        <form method="POST" action="{{ route('data-migration.store') }}">
            ...
            <div class="mb-4">
                ...
                <input type="text" 
                       ...
                       value="{{ old('name') }}"
                       class="form-control"
                       placeholder="e.g., Client Data Migration - {{ date('Y-m-d') }}"
                       required>
            ...
            <div class="mb-4">
                ...
                <select name="batch_size" id="batch_size" class="form-select">
                    <option value="25" {{ old('batch_size') == '25' ? 'selected' : '' }}>25 items per batch</option>
                    <option value="50" {{ old('batch_size') == '50' ? 'selected' : '' }}>50 items per batch</option>
                    <option value="100" {{ old('batch_size', '100') == '100' ? 'selected' : '' }}>100 items per batch (recommended)</option>
                </select>
                <div class="form-text">
...
<div class="alert alert-info mt-4">
    <div class="d-flex">
        ...
        <div class="ms-3">
            ...
            <ul class="mb-0">
                <li><strong>Batch Processing:</strong> Data is fetched and stored in configurable batches to ensure reliability</li>
                <li><strong>Asynchronous:</strong> Migrations run in the background using Laravel's queue system</li>
                <li><strong>Progress Tracking:</strong> Real-time progress updates show completion status for each batch</li>
                <li><strong>Error Recovery:</strong> Failed batches can be retried without affecting successful ones</li>
                <li><strong>Data Verification:</strong> Compare migrated data against the original API data for accuracy</li>
            </ul>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updatePreview() {
        ...
        let preview = '<div>';
        preview += `<p><strong>Resources:</strong> ${selectedResources.map(r => r.charAt(0).toUpperCase() + r.slice(1)).join(', ')}</p>`;
        ...
        preview += `<p><strong>Processing:</strong> Each resource type will be processed in separate batches of ${batchSize} items</p>`;
