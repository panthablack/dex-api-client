...
import { getScreenshotPath } from './helpers/generic'
...
test.describe('Verification Simple Tests', () => {
  ...
  test('Quick Verify should show meaningful content or proper no-data message', async ({
    page,
  }) => {
    // Click Quick Verify
    ...
    const hasProperNoData =
      modalContent.includes('Sample Size') && modalContent.includes('No data to verify')
    const hasProperError =
      modalContent.includes('Verification Failed') && modalContent.includes('Error:')
    ...
    await page.screenshot({
      path: getScreenshotPath('./quick-verify-result.png'),
    })
  })
