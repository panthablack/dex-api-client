...
try {
  ...
  const jsChecks = [
    ...
    {
      test: 'showUpdateForm method present',
      pattern: /showUpdateForm\(resourceType, resourceId\)/,
    },
    { test: 'confirmDelete method present', pattern: /confirmDelete\(resourceType, resourceId\)/ },
    {
      test: 'CSRF token handling',
      pattern: /'X-CSRF-TOKEN': document\.querySelector\('meta\[name="csrf-token"\]'\)/,
    },
...
try {
  ...
  const issueChecks = [
    {
      test: 'No PHP errors visible',
      pattern: /<\?php|Fatal error|Parse error/,
      shouldMatch: false,
    },
    {
      test: 'No obvious JavaScript syntax errors',
